...
（予めSDカードの/boot直下に"ssh"という名前の空ファイルを配置しておけば、SSH Serverが有効状態で起動する。この方法だとHDMI接続のディスプレイなしでTFTディスプレイに映像を出せる。Rasbianの初期ユーザ名：pi パスワード:raspberry。VNCサーバはsudo raspi-configで有効に設定可能。そのままだと画面が出ないので、/boot/config.txtの#hdmi_force_hotplug=1をコメントアウトすると良い。）  
raspi-config  
```
